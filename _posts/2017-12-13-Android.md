---
layout: post  #这个不变
title: "调用系统相机、相册、剪裁图片并上传（常用于上传头像，兼容Android7.0）" #标题
date: 2017-12-13 16:13 #时间
description: "昨天写了关于调用相册并裁剪图片，今天学习了其他前辈的东西做一些补充"  #说明
tag: Android #这是分类标签
---
...
（2）接下来，我们需要在资源(res)目录下创建一个xml目录，并建立一个以上面名字为文件名的xml文件，内容如下：
```
...
（1）在这之前，我们需要在AndroidManifest.xml中增加必要的读写权限：
```
...
(1)我们在onActivityResult方法中获得返回的图片信息,在这里我们会先调用剪裁去剪裁图片,然后对剪裁返回的图片进行设置、保存、上传等操作
